...
{
    /// <summary>
    /// Алгоритмом DES в режиме OFB
    /// </summary>
    internal class EncBaseLab7 : DES
    {
        // Начальный вектор
        ...
        public EncBaseLab7(string key, string initialVector, int bitsCount) : base(key)
        {
        ...
        public override string Encrypt(string msg)
        {
            ...
            foreach (var bitArray in GetBitArrays(msg, bitsCount))
            {
                ...
                // Сдвиг временного вектора на t (bitsCount) битов влево, заполняя справа первыми t битами desResult
                var vector = tempVector.ToList();
                var index = 63;
                for (var i = bitsCount - 1; i >= 0; i--)
                {
                    tempVector[index--] = desResult[i];
                }
                var vectorIndex = 63;
                while (index >= 0)
                {
